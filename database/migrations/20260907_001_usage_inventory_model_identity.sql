...
SET @add_non_asset_usage_model_key_sql = IF(
  ...
  'ALTER TABLE employee_non_asset_usage ADD COLUMN inventory_model_key BIGINT UNSIGNED GENERATED ALWAYS AS (COALESCE(inventory_model_id, 0)) STORED AFTER inventory_model_id',
  'SELECT 1'
);
...
SET @add_monitor_usage_model_key_sql = IF(
  ...
  'ALTER TABLE employee_monitor_usage ADD COLUMN inventory_model_key BIGINT UNSIGNED GENERATED ALWAYS AS (COALESCE(inventory_model_id, 0)) STORED AFTER inventory_model_id',
  'SELECT 1'
);
