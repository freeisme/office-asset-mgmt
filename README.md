...
`v2.0.3` 是面向内部 IT 资产运营和 ITIL 服务管理的单体 Web 系统。仓库不包含业务数据、数据库备份、账户密码、令牌或证书。
...
2. 确认数据库已经包含 `legacy-20260813` 的核心业务与认证结构。`v2.0.3` 会以首条增量迁移补齐旧版缺少的认证启动保护表和会话来源字段，无需手工执行历史重建 SQL。
