param(
  ...
  [int]$Port = 3306,
  [string]$Database = "office_asset_mgmt",
  [Alias("DbName")][string]$DbName,
  [string]$DbPassword,
...
$migrationRunner = Join-Path $repoRoot "migration_runner.py"
$mysqlRunner = Join-Path $repoRoot "run_mysql_utf8.py"
...
function Invoke-SqlFile {
  ...
  Write-Host "Applying database/$FileName ..."
  & $Python $mysqlRunner `
    --mysql $Mysql `
    --user $User `
    --host $HostName `
    ...
    --file $path
  if ($LASTEXITCODE -ne 0) {
    throw "MySQL failed while applying database/$FileName."
  }
}
...
if ($isNewInstallation) {
  if ($Reinitialize) {
    Write-Warning "Reinitialize requested: database/01_schema.sql rebuilds tables and removes existing business data."
  }
